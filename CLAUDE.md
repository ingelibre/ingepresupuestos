...
**Autor:** Ing. Marco Sumari · **Software propietario** (© Sumari SAC) · Versión actual: **2.9.0** (primer release propietario, publicado 2026-08-08)
...
>
> **Emisión de claves — 3 vías, misma firma:** `./generar-licencia.sh` (CLI, raíz del repo), `scripts/gen_license.py`, o el **generador GRÁFICO** en `~/.ingepresupuestos-licencias/generador.py` (menú de apps → «IngePresupuestos — Licencias»; reusa `gen_license.py` vía importlib, no duplica crypto). **`~/.ingepresupuestos-licencias/` es la carpeta maestra del negocio** (clave privada + `emitidas.csv` + generador) — el botón «Respaldar carpeta…» del generador la zipea; el respaldo externo debe ir CIFRADO (7z -p -mhe=on). Perderla = no poder emitir nunca más.
>
...
- **Iconografía:** `utils/icons.py::icon("alias")`. NO emojis para UI. El set es **Tabler Icons (MIT)** con los NOMBRES de archivo históricos de elementary (la carpeta `elementary/24/` se llama así solo por compatibilidad). Icono nuevo = Tabler outline con `stroke="#37474F"`; **NUNCA copiar iconos de temas del sistema** (son GPL/CC-BY-SA — ya pasó y hubo que reemplazar 49). Ver `resources/icons/PROCEDENCIA.md`. **Pendiente del autor (2026-08-08): no le convencen los Tabler de trazo** — quiere evaluar sets de estilo lleno (Material Symbols filled / Phosphor fill / Fluent filled); comparación en `../backups/iconos-comparacion/` (los antiguos elementary ahí son GPL: SOLO referencia visual, no pueden volver).
